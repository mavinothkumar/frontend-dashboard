...
function fed_call_function_method( $item ) {
	if ( is_string( $item['callable'] ) && function_exists( $item['callable'] ) ) {
		...
		?>
        <div class="container fed_add_page_profile_container">
			<?php
			$message = is_array( $item['callable'] ) ? $item['callable']['method'] : $item['callable'];
			fed_show_alert_message( sprintf( __( 'OOPS! You have not add the callable function ( %s ), please add to show the body container', 'frontend-dashboard' ), esc_html( $message ) ) );

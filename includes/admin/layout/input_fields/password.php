...
function fed_admin_input_fields_password( $row, $action, $menu_options ) {
	$is_active = ( isset( $row['input_type'] ) && 'password' === $row['input_type'] );
	?>
	<div class="fed_input_type_container fed_input_password_container space-y-7 <?php echo $is_active ? '' : 'hide hidden'; ?>" data-field-type="password">
		<form method="post"
			...
			<div class="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-xs space-y-6 sm:space-y-7">
				<div class="flex items-center gap-3.5 pb-5 border-b border-slate-100">
					...
					<div>
						<h3 class="text-sm sm:text-base font-bold text-slate-900 m-0"><?php esc_html_e( 'Password Field', 'frontend-dashboard' ); ?></h3>
						<p class="text-xs text-slate-500 m-0 mt-0.5"><?php esc_html_e( 'Secure masked password input field.', 'frontend-dashboard' ); ?></p>
					</div>
				</div>

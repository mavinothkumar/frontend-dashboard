<?php
/**
 * User Profile Fields Management.
 *
			...
			<div class="bc_fed max-w-7xl mx-auto px-4 sm:px-6 py-8 font-sans">
				<div class="p-6 rounded-2xl bg-rose-50 border border-rose-200 text-rose-700 flex items-center gap-3">
					...
					<div>
						...
						<p class="text-xs m-0"><?php echo esc_html( $user_profile->get_error_message() ); ?></p>
					</div>
				</div>
			</div>
			<?php
...
if ( ! function_exists( 'fed_get_profile_field_type_icon' ) ) {
	/**
	 * Helper function to return icon for specific field types.
	 *
	 * @param string $type Field type.
	 * @return string FontAwesome class.
	 */
	function fed_get_profile_field_type_icon( $type ) {
		switch ( $type ) {
			case 'single_line':
				return 'fas fa-font';
			case 'multi_line':
...
if ( ! function_exists( 'fed_get_user_profile_menu_items' ) ) {
	/**
	 * Render User Profile Menu Items and Fields UI.
	 *
	 * @param array $profiles User profile fields from database.
	 */
	function fed_get_user_profile_menu_items( $profiles ) {
		...
		$_all_menus = fed_fetch_menu();
		if ( ! is_array( $_all_menus ) ) {
			$_all_menus = array();
		}
		usort( $_all_menus, 'fed_sort_by_order' );

		// Filter out menus that are configured as WordPress Pages or External URLs
		$eligible_menus = array_filter(
			$_all_menus,
			function ( $m ) {
				$key = isset( $m['menu_key'] ) ? $m['menu_key'] : '';
				return ( 'yes' !== $key && 'url' !== $key );
			}
		);

		if ( empty( $eligible_menus ) ) {
			$eligible_menus = $_all_menus;
		}

		$menus_map = fed_get_key_value_array( $eligible_menus, 'menu_slug' );

		// Group fields by menu slug
		$group_by = array();
		foreach ( $menus_map as $slug => $m_data ) {
			$group_by[ $slug ] = array();
		}

		foreach ( $profiles as $profile ) {
			$menu_slug = isset( $profile['menu'] ) && ! empty( $profile['menu'] ) ? $profile['menu'] : 'profile';
			if ( isset( $group_by[ $menu_slug ] ) ) {
		...
		// Sort fields within each menu group
		foreach ( $group_by as $slug => &$fields_list ) {
			usort( $fields_list, 'fed_sort_by_order' );
		}
		unset( $fields_list );

		$total_fields_count = count( $profiles );
		$total_menus_count  = count( $eligible_menus );
		$nonce              = wp_create_nonce( 'fed_nonce' );
		$ajax_url           = admin_url( 'admin-ajax.php' );
		$add_new_field_url  = add_query_arg( array( 'fed_action' => 'profile' ), menu_page_url( 'fed_user_profile', false ) );
		...
		<div class="bc_fed fed-admin-wrap w-full max-w-none px-4 sm:px-8 py-6 sm:py-8 font-sans text-slate-800" data-nonce="<?php echo esc_attr( $nonce ); ?>" data-ajax-url="<?php echo esc_url( $ajax_url ); ?>">
				...
				<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 relative z-10">
					<div class="flex items-center gap-3.5">
						<div class="w-10 h-10 rounded-2xl bg-indigo-600 text-white flex items-center justify-center shadow-xs shrink-0" style="background-color: #4f46e5 !important; color: #ffffff !important;">
							<i class="fas fa-id-card text-sm" style="color: #ffffff !important;"></i>
						</div>
						<div>
							<div class="flex items-center gap-2.5">
								<h1 class="text-base sm:text-lg font-bold text-slate-900 tracking-tight m-0 p-0">
									<?php esc_html_e( 'User Profile Form Fields', 'frontend-dashboard' ); ?>
								</h1>
							...
							<p class="text-xs text-slate-500 m-0 mt-0.5 font-medium">
								<?php esc_html_e( 'Manage, customize, and arrange form fields organized by dashboard navigation menus.', 'frontend-dashboard' ); ?>
							</p>
						</div>
					</div>
					...
					<div class="flex items-center gap-2.5 shrink-0">
						<a href="<?php echo esc_url( $dashboard_menu_url ); ?>" class="fed-btn-secondary h-10 inline-flex items-center justify-center gap-2 px-4 rounded-xl font-semibold text-xs transition-all cursor-pointer shadow-2xs no-underline">
							<i class="fas fa-bars text-xs"></i>
							<span><?php esc_html_e( 'Dashboard Menus', 'frontend-dashboard' ); ?></span>
						</a>
						<a href="<?php echo esc_url( $add_new_field_url ); ?>" class="fed-open-add-field-modal fed-btn-primary h-10 inline-flex items-center justify-center gap-2 px-5 rounded-xl font-semibold text-xs transition-all active:scale-95 cursor-pointer shadow-sm no-underline" data-menu="profile">
							<i class="fas fa-plus text-xs" style="color: #ffffff !important;"></i>
							<span style="color: #ffffff !important;"><?php esc_html_e( 'Add New Field', 'frontend-dashboard' ); ?></span>
						</a>
					</div>
				</div>
			...
			<!-- Main Full-Width Flex Layout (Sidebar Menu Tabs + Content Fields Panel) -->
			<?php
			$req_payload     = array_merge( \FED\Helpers\InputHelper::get(), \FED\Helpers\InputHelper::post() );
			$active_tab_slug = isset( $req_payload['tab'] ) && ! empty( $req_payload['tab'] ) ? esc_attr( $req_payload['tab'] ) : ( isset( $eligible_menus[0]['menu_slug'] ) ? $eligible_menus[0]['menu_slug'] : 'profile' );
			?>
			<div class="flex flex-col lg:flex-row gap-6 items-start">
				<!-- LEFT SIDEBAR: Navigation Menu Tabs -->
				<div class="w-full lg:w-72 xl:w-80 shrink-0 space-y-3">
					<div class="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-xs space-y-2">
						<div class="px-2 py-1.5 flex items-center justify-between border-b border-slate-100 pb-2.5">
							<span class="text-[11px] font-bold uppercase tracking-wider text-slate-500">
								<?php esc_html_e( 'Menu Locations', 'frontend-dashboard' ); ?>
							</span>
							<span class="text-[11px] font-semibold text-slate-400">
								<?php echo (int) $total_menus_count; ?> <?php esc_html_e( 'Active', 'frontend-dashboard' ); ?>
							</span>
						</div>

						<!-- Menu Tab Buttons List -->
						<div class="space-y-1.5 pt-1" id="fed_menu_tabs_nav">
							<?php
							$tab_idx = 0;
							foreach ( $eligible_menus as $menu_item ) :
								$m_slug     = esc_attr( $menu_item['menu_slug'] );
								$m_name     = esc_html( $menu_item['menu'] );
								$m_icon     = ! empty( $menu_item['menu_image_id'] ) ? esc_attr( $menu_item['menu_image_id'] ) : 'fas fa-bars';
								$field_cnt  = isset( $group_by[ $m_slug ] ) ? count( $group_by[ $m_slug ] ) : 0;
								$is_default = ( $m_slug === $active_tab_slug );
								?>
								<button type="button"
									...
									data-tab-target="fed_tab_content_<?php echo esc_attr( $m_slug ); ?>"
									data-menu-slug="<?php echo esc_attr( $m_slug ); ?>">
									<div class="flex items-center gap-3 min-w-0">
										<div class="fed-tab-icon w-8 h-8 rounded-lg flex items-center justify-center text-xs shrink-0 transition-colors <?php echo $is_default ? 'bg-indigo-600 text-white' : 'bg-white border border-slate-200/80 text-slate-500 group-hover:text-indigo-600 group-hover:border-indigo-200'; ?>">
											<i class="<?php echo esc_attr( $m_icon ); ?>"></i>
										</div>
					...
					<div class="p-4 rounded-2xl bg-indigo-50/60 border border-indigo-100 text-xs text-indigo-950 space-y-2">
						<div class="flex items-center gap-2 font-bold text-indigo-900">
							...
							<span><?php esc_html_e( 'Menu Organization', 'frontend-dashboard' ); ?></span>
						</div>
						<p class="text-[11px] text-indigo-800/80 m-0 leading-relaxed">
							<?php esc_html_e( 'Only standard menus are listed here. Menus configured as external links or page overrides do not contain user profile fields.', 'frontend-dashboard' ); ?>
						</p>
					</div>
				</div>

				<!-- RIGHT CONTENT PANEL: Form Fields per Menu Tab (Full Width Responsive) -->
				<div class="flex-1 min-w-0 w-full">
					...
					foreach ( $eligible_menus as $menu_item ) :
						...
						$m_name     = esc_html( $menu_item['menu'] );
						$m_icon     = ! empty( $menu_item['menu_image_id'] ) ? esc_attr( $menu_item['menu_image_id'] ) : 'fas fa-bars';
						$m_fields   = isset( $group_by[ $m_slug ] ) ? $group_by[ $m_slug ] : array();
						$is_visible = ( $m_slug === $active_tab_slug );
						?>
						<div id="fed_tab_content_<?php echo esc_attr( $m_slug ); ?>" class="fed-tab-pane space-y-4 <?php echo $is_visible ? '' : 'hidden'; ?>" data-menu-slug="<?php echo esc_attr( $m_slug ); ?>">
							<!-- Section Header Card -->
									...
									<div>
										<div class="flex items-center gap-2.5">
											<h2 class="text-sm sm:text-base font-bold text-slate-900 m-0 p-0">
												<?php echo esc_html( $m_name ); ?> <?php esc_html_e( 'Fields', 'frontend-dashboard' ); ?>
											</h2>
											<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100">
										...
										<p class="text-[11px] text-slate-500 m-0 mt-0.5 font-medium">
											...
											printf( esc_html__( 'Form fields rendered inside the "%s" dashboard menu section.', 'frontend-dashboard' ), esc_html( $m_name ) );
											?>
										</p>
									</div>
								</div>
							...
							<div class="fed-fields-sortable space-y-3"
								data-menu="<?php echo esc_attr( $m_slug ); ?>"
										...
										<div class="fed-field-card bg-white rounded-xl py-3.5 px-4 sm:px-6 border border-slate-200/90 shadow-2xs hover:shadow-xs hover:border-slate-300 transition-all flex flex-col xl:flex-row xl:items-center justify-between gap-3.5 group cursor-default"
											...
											<div class="flex items-center gap-3.5 min-w-0 flex-1">
												...
												<div class="flex flex-wrap items-center gap-2.5 min-w-0 flex-1">
													...
													<!-- Meta Badge -->
													<span class="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-mono bg-slate-100 text-slate-600 border border-slate-200/70 font-medium shrink-0 whitespace-nowrap">
														meta: <?php echo esc_html( $field_meta ); ?>
													</span>

													<!-- Field Type Pill -->
													<span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-semibold bg-slate-100 text-slate-500 border border-slate-200/60 uppercase shrink-0 whitespace-nowrap">
														<?php echo esc_html( str_replace( '_', ' ', $field_type ) ); ?>
													</span>

													<!-- Core vs Custom Pill -->
													<?php if ( ! $is_extra ) : ?>
														<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-purple-50 text-purple-700 border border-purple-100 shrink-0 whitespace-nowrap">
															<i class="fas fa-shield-alt text-[9px]"></i> <?php esc_html_e( 'Core', 'frontend-dashboard' ); ?>
														</span>
													<?php else : ?>
														<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-100 shrink-0 whitespace-nowrap">
											...
											<!-- Right: Visibility Badges & Actions (Clean horizontal row) -->
											<div class="flex items-center justify-between sm:justify-end gap-3.5 shrink-0 pt-2 xl:pt-0 border-t xl:border-t-0 border-slate-100 whitespace-nowrap">
												<!-- 3 Visibility Status Badges -->
												<div class="flex items-center gap-1.5 shrink-0">
													<!-- Register Visibility -->
													<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[11px] font-semibold transition-all <?php echo $is_reg_active ? 'bg-emerald-50 text-emerald-700 border border-emerald-200/80' : 'bg-slate-100 text-slate-400 border border-slate-200/50 opacity-60'; ?>" title="<?php echo $is_reg_active ? esc_attr__( 'Enabled on Registration Form', 'frontend-dashboard' ) : esc_attr__( 'Disabled on Registration Form', 'frontend-dashboard' ); ?>">
														<i class="fas fa-<?php echo $is_reg_active ? 'check' : 'times'; ?> text-[8px]"></i>
														<span><?php esc_html_e( 'Register', 'frontend-dashboard' ); ?></span>
													</span>

													<!-- Dashboard Visibility -->
													<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[11px] font-semibold transition-all <?php echo $is_dash_active ? 'bg-indigo-50 text-indigo-700 border border-indigo-200/80' : 'bg-slate-100 text-slate-400 border border-slate-200/50 opacity-60'; ?>" title="<?php echo $is_dash_active ? esc_attr__( 'Enabled on User Dashboard', 'frontend-dashboard' ) : esc_attr__( 'Disabled on User Dashboard', 'frontend-dashboard' ); ?>">
														<i class="fas fa-<?php echo $is_dash_active ? 'check' : 'times'; ?> text-[8px]"></i>
														<span><?php esc_html_e( 'Dashboard', 'frontend-dashboard' ); ?></span>
													</span>

													<!-- Profile Visibility -->
													<span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[11px] font-semibold transition-all <?php echo $is_prof_active ? 'bg-blue-50 text-blue-700 border border-blue-200/80' : 'bg-slate-100 text-slate-400 border border-slate-200/50 opacity-60'; ?>" title="<?php echo $is_prof_active ? esc_attr__( 'Enabled on User Profile', 'frontend-dashboard' ) : esc_attr__( 'Disabled on User Profile', 'frontend-dashboard' ); ?>">
														<i class="fas fa-<?php echo $is_prof_active ? 'check' : 'times'; ?> text-[8px]"></i>
														<span><?php esc_html_e( 'Profile', 'frontend-dashboard' ); ?></span>
													</span>
												</div>
												...
												<div class="flex items-center gap-2 shrink-0">
													<!-- Edit Button -->
													<a href="<?php echo esc_url( $edit_url ); ?>" class="fed-open-edit-field-modal fed-btn-edit h-8.5 inline-flex items-center justify-center gap-1.5 px-3.5 rounded-lg font-semibold text-xs transition-all cursor-pointer no-underline shadow-2xs" data-id="<?php echo esc_attr( $field_id ); ?>" data-menu="<?php echo esc_attr( $m_slug ); ?>">
														<i class="fas fa-edit text-[11px]"></i>
														<span><?php esc_html_e( 'Edit', 'frontend-dashboard' ); ?></span>
													</a>

													<!-- Delete Button (Only for custom extra fields) -->
													<?php if ( $is_extra ) : ?>
														<button type="button"
															...
															title="<?php esc_attr_e( 'Delete this field', 'frontend-dashboard' ); ?>">
		...
		<div id="fed_delete_field_modal" class="fixed inset-0 z-50 overflow-y-auto hidden flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs transition-opacity duration-200" style="z-index: 999999 !important;">
			<div class="delete-modal-content bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-slate-100 transform scale-95 opacity-0 transition-all duration-200 text-center">
				...
				<h3 class="text-base sm:text-lg font-bold text-slate-900 mb-1" id="fed_delete_field_title">
					<?php esc_html_e( 'Delete Form Field?', 'frontend-dashboard' ); ?>
				</h3>
				<p class="text-xs sm:text-sm text-slate-500 mb-6 leading-relaxed" id="fed_delete_field_desc">
					<?php esc_html_e( 'Are you sure you want to delete this custom profile field? All associated user meta will be permanently deleted.', 'frontend-dashboard' ); ?>
				</p>
		...
		(function($) {
			...
			$(document).ready(function() {
				...
				function activateTab(targetId) {
					...
					try {
						sessionStorage.setItem('fed_active_tab_user_profile', slug);
				...
				try {
					...
					var initialTabSlug = urlParams.get('tab') || sessionStorage.getItem('fed_active_tab_user_profile');
				...
				function filterFields(query) {
					...
					var activeSlug = $activePane.data('menu-slug');
					...
					$allPanes.each(function() {
						...
						$('.fed-menu-tab-btn[data-menu-slug="' + slug + '"] .fed-tab-count').text(paneMatches);

						if (slug !== activeSlug && paneMatches > 0) {
							var menuName = $('.fed-menu-tab-btn[data-menu-slug="' + slug + '"]').find('.text-xs').text().trim();
					...
					if (activeMatches === 0) {
						...
						if (otherMatchesInfo.length > 0) {
							var hintsHtml = '<span>Found in other menus: </span>';
							...
							$noResults.find('.fed-other-tabs-match-hint').text('No fields match "' + query + '" across any menu location.');
						}
					} else {
				...
				$(document).on('click', '.fed-trigger-delete-field', function(e) {
					...
					$('#fed_delete_field_modal').removeClass('hidden');
					setTimeout(function() {
				...
				function closeDeleteModal() {
					...
					setTimeout(function() {
						$('#fed_delete_field_modal').addClass('hidden');
				...
				$('#fed_confirm_delete_field_btn').on('click', function() {
					...
					$.ajax({
						...
						data: {
							...
							data: 'profile_id=' + fieldId + '&profile_name=' + encodeURIComponent(fieldName) + '&fed_nonce=' + nonce
						},
						success: function(response) {
							$loader.addClass('hidden');
							if (response && response.success) {
								if ($row && $row.length) {
									$row.fadeOut(300, function() {
										...
										var menuSlug = $pane.data('menu-slug');
										$row.remove();

										// Update counters
										var remaining = $pane.find('.fed-field-card').length;
										$pane.find('.fed-tab-fields-counter').text(remaining);
										$('.fed-menu-tab-btn[data-menu-slug="' + menuSlug + '"] .fed-tab-count').text(remaining);
									});
								}
								showToast((response.data && response.data.message) ? response.data.message : 'Field deleted successfully.', false);
							} else {
						...
						error: function() {
							$loader.addClass('hidden');
							showToast('Server error while deleting field.', true);
						}
					});
				});
				...
				function openBuilderModal(fieldId, menuSlug, $triggerBtn) {
					showLoading(
						fieldId ? 'Loading Field Settings...' : 'Opening Form Builder...',
						fieldId ? 'Fetching form field configurations & role permissions.' : 'Preparing new field workspace.'
					);
				...
				$(document).on('click', '.fed-open-add-field-modal', function(e) {
					e.preventDefault();
					var $btn = $(this);
					var menuSlug = $btn.data('menu') || $('.fed-tab-pane:not(.hidden)').data('menu-slug') || 'profile';
				...
				$(document).on('click', '.fed-open-edit-field-modal', function(e) {
					...
					openBuilderModal(fieldId, menuSlug, $btn);
				});
				...
				$(document).off('submit.fed_modal_builder').on('submit.fed_modal_builder', '#fed_field_builder_modal form.fed_ajax', function(e) {
					...
					$.ajax({
						...
						success: function(response) {
							...
							if (isSuccess) {
								closeBuilderModal();
								var savedMenuSlug = form.find('input[name="menu"]').val() || $('.fed-menu-tab-btn.is-active').data('menu-slug') || 'profile';
								try {
									sessionStorage.setItem('fed_active_tab_user_profile', savedMenuSlug);
								} catch(e) {}

								// Smooth reload to show newly updated fields in list
								setTimeout(function() {
									var reloadUrl = new URL(window.location.href);
									reloadUrl.searchParams.set('tab', savedMenuSlug);

...
function fed_get_dashboard_menu_items() {
	...
	<style>
		...
		#fed_modal_cancel_btn {
			background-color: #ffffff !important;
			color: #334155 !important;
			border: 1px solid #cbd5e1 !important;
		}
		#fed_modal_cancel_btn:hover {
			background-color: #f8fafc !important;
	...
	<div class="bc_fed fed-admin-wrap max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 font-sans text-slate-800">
		<?php echo fed_loader(); ?>

		<!-- Toast Notification Element -->
		<div id="fed_toast_notification" class="fixed bottom-6 right-6 z-50 transform translate-y-16 opacity-0 transition-all duration-300 pointer-events-none flex items-center gap-3 bg-slate-900 text-white px-5 py-3 rounded-xl shadow-xl border border-slate-700">
		...
		<!-- Page Header & Action Bar (Slim & Streamlined) -->
		<div class="bg-white rounded-2xl p-4 sm:p-5 shadow-xs border border-slate-200/80 mb-5 relative overflow-hidden">
			<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 relative z-10">
				<div class="flex items-center gap-2.5">
					<div class="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-indigo-600 text-white flex items-center justify-center shadow-xs shrink-0" style="background-color: #4f46e5 !important; color: #ffffff !important;">
						<i class="fas fa-bars text-xs" style="color: #ffffff !important;"></i>
					</div>
				...
				<div class="flex items-center gap-3 shrink-0">
					<button type="button" id="fed_open_add_menu_btn" class="fed-btn-primary h-9 inline-flex items-center justify-center gap-2 px-4 rounded-xl font-semibold text-xs transition-all active:scale-95 cursor-pointer shadow-xs">
			...
			<div class="mt-3.5 pt-3.5 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
				...
				<div class="flex items-center gap-2 text-xs font-medium text-slate-500 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-100">
		...
		<!-- Reorderable List Container (Comfortable & Breathable) -->
		<div id="fed_dashboard_menu_sortable" class="space-y-3.5" data-nonce="<?php echo esc_attr( $nonce ); ?>" data-url="<?php echo esc_url( $ajax_url ); ?>">
			...
			if ( ! empty( $menus ) ) {
				foreach ( $menus as $menu ) {
					...
					?>
					<div class="fed-menu-card bg-white rounded-xl py-3.5 px-4 sm:px-6 border border-slate-200/90 shadow-2xs hover:shadow-xs hover:border-slate-300 transition-all flex items-center justify-between gap-4 group cursor-default"
						...
						<div class="fed-card-left flex items-center gap-3.5 min-w-0 flex-1">
							...
							<!-- Title & Meta Badges (Precision Vertical Alignment) -->
							<div class="fed-card-details flex flex-wrap items-center gap-2.5 min-w-0 flex-1">
								<span class="fed-menu-title text-sm font-bold text-slate-800 truncate">
									<?php echo esc_html( $menu['menu'] ); ?>
								</span>

								<!-- Slug Badge -->
								<span class="fed-badge-item inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-mono bg-slate-100 text-slate-600 border border-slate-200/60 font-medium">
									slug: <?php echo esc_html( $menu_slug ); ?>
								</span>

								<!-- System vs Custom Badge -->
								<?php if ( $is_system_core ) : ?>
									<span class="fed-badge-item inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-purple-50 text-purple-700 border border-purple-100">
										<i class="fas fa-shield-alt text-[9px]"></i> <?php esc_html_e( 'System', 'frontend-dashboard' ); ?>
									</span>
								<?php else : ?>
									<span class="fed-badge-item inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-100">
										<i class="fas fa-star text-[9px]"></i> <?php esc_html_e( 'Custom', 'frontend-dashboard' ); ?>
									</span>
								<?php endif; ?>

								<!-- Profile Status Indicator -->
								<span class="fed-badge-item hidden md:inline-flex items-center gap-1.5 text-[11px] font-medium text-slate-400 pl-1">
								...
								<!-- Roles Summary Pill -->
								<span class="fed-badge-item hidden lg:inline-flex items-center gap-1.5 text-[11px] text-slate-500 font-medium pl-1">
		...
		<!-- ========================================== -->
		<!-- MODAL: ADD / EDIT DASHBOARD MENU           -->
		<!-- ========================================== -->
		<div id="fed_menu_modal" class="hidden fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 transition-opacity duration-200">
			<div class="bg-white rounded-2xl shadow-2xl border border-slate-200/90 w-full max-w-xl overflow-hidden transform transition-all duration-300 scale-95 opacity-0 modal-content-box my-6">
				<!-- Modal Header -->
				<div class="px-6 sm:px-7 py-4.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/70">
					<div class="flex items-center gap-3">
						<div id="fed_modal_icon_box" class="w-9 h-9 rounded-xl bg-indigo-600 text-white flex items-center justify-center shadow-xs shrink-0" style="background-color: #4f46e5 !important; color: #ffffff !important;">
							<i class="fas fa-plus text-xs" style="color: #ffffff !important;"></i>
						</div>
						<div>
							<h3 id="fed_modal_title" class="text-base font-bold text-slate-900 m-0">
								<?php esc_html_e( 'Add New Dashboard Menu', 'frontend-dashboard' ); ?>
							</h3>
							<p id="fed_modal_subtitle" class="text-[11px] text-slate-500 m-0 font-medium">
								<?php esc_html_e( 'Configure title, icon, and role permissions.', 'frontend-dashboard' ); ?>
							</p>
						</div>
					</div>
					<button type="button" id="fed_modal_close_btn" class="w-8 h-8 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 flex items-center justify-center transition-colors cursor-pointer">
						<i class="fas fa-times text-sm"></i>
					</button>
				</div>

				<!-- Modal Body Form -->
				<form id="fed_modal_menu_form" class="p-6 sm:p-7">
					<input type="hidden" name="menu_id" id="fed_form_menu_id" value="" />
					<input type="hidden" name="fed_nonce" value="<?php echo esc_attr( $nonce ); ?>" />

					<!-- Row 1: Name & Slug -->
					<div class="fed-modal-field-group grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
						<div>
						...
						<div>
							...
							<p id="fed_slug_hint" class="text-[10px] text-slate-400 mt-1.5 mb-0 leading-tight">
								<?php esc_html_e( 'Unique identifier used for URL binding.', 'frontend-dashboard' ); ?>
							</p>
						</div>
					</div>

					<!-- Row 2: Icon Picker & Order -->
					<div class="fed-modal-field-group grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
						<!-- Icon Field -->
						<div>
							<label class="block text-[11px] font-bold uppercase tracking-wider text-slate-700 mb-1.5">
					...
					<!-- Toggle: User Profile Visibility (Comfortable Card) -->
					<div class="fed-modal-field-group p-3.5 sm:p-4 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
						<div>
							<h4 class="text-xs sm:text-sm font-semibold text-slate-800 m-0 mb-0.5">
					...
					<!-- ==================================================== -->
					<!-- SCALABLE USER ROLE ACCESS PERMISSIONS SELECTOR        -->
					<!-- ==================================================== -->
					<div class="fed-modal-field-group border border-slate-200/90 rounded-xl p-4 bg-white shadow-2xs space-y-3">
						<!-- Role Mode Selector (All vs Specific) -->
						<div class="flex items-center justify-between gap-2 pb-2.5 border-b border-slate-100">
							...
							<div class="inline-flex p-1 bg-slate-100 rounded-lg shrink-0">
								<button type="button" id="fed_role_mode_all_btn" class="px-3 py-1 rounded-md text-xs font-semibold transition-all bg-white text-indigo-700 shadow-2xs cursor-pointer">
									<i class="fas fa-globe mr-1 text-[10px]"></i> <?php esc_html_e( 'All Roles', 'frontend-dashboard' ); ?>
								</button>
								<button type="button" id="fed_role_mode_specific_btn" class="px-3 py-1 rounded-md text-xs font-semibold text-slate-600 hover:text-slate-900 transition-all cursor-pointer">
									<i class="fas fa-user-lock mr-1 text-[10px]"></i> <?php esc_html_e( 'Specific Roles', 'frontend-dashboard' ); ?>
								</button>
							</div>
						</div>

						<!-- Specific Roles Container (Collapsible & Searchable) -->
						<div id="fed_specific_roles_wrapper" class="space-y-2.5 pt-1 hidden">
							...
							<div class="flex items-center justify-between gap-3">
								<div class="relative flex-1">
									...
									<input type="text" id="fed_role_search_filter" placeholder="<?php echo esc_attr( sprintf( __( 'Filter %d roles...', 'frontend-dashboard' ), $total_roles_count ) ); ?>" class="w-full pr-2.5 py-1.5 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-800 placeholder:text-slate-400 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-100 transition-all outline-none font-medium" style="padding-left: 30px !important;" />
								</div>

								<div class="flex items-center gap-2 shrink-0">
									<span id="fed_role_selected_count" class="text-[10px] font-semibold px-2.5 py-1 rounded bg-indigo-50 text-indigo-700 border border-indigo-100">
										0 / <?php echo $total_roles_count; ?>
									</span>
									<button type="button" id="fed_roles_select_all" class="text-xs font-semibold text-indigo-600 hover:text-indigo-800 transition-colors cursor-pointer px-1 py-0.5">
										<?php esc_html_e( 'All', 'frontend-dashboard' ); ?>
									</button>
									<span class="text-slate-300 text-xs">|</span>
									<button type="button" id="fed_roles_deselect_all" class="text-xs font-semibold text-slate-500 hover:text-slate-700 transition-colors cursor-pointer px-1 py-0.5">
										<?php esc_html_e( 'None', 'frontend-dashboard' ); ?>
									</button>
								</div>
							</div>

							<!-- Role Chips Grid (Comfortable Chips) -->
							<div id="fed_roles_chips_container" class="flex flex-wrap gap-2 max-h-36 overflow-y-auto p-2 border border-slate-100 rounded-lg bg-slate-50/50">
								<?php foreach ( $user_roles as $role_key => $role_name ) : ?>
									<label class="fed-role-chip inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer select-none" data-role-name="<?php echo esc_attr( strtolower( $role_name ) ); ?>" data-role-key="<?php echo esc_attr( strtolower( $role_key ) ); ?>">
										<input type="checkbox" name="user_role[<?php echo esc_attr( $role_key ); ?>]" value="Enable" class="fed-role-checkbox" checked />
										<span class="truncate max-w-[140px]"><?php echo esc_html( $role_name ); ?></span>
									</label>
								<?php endforeach; ?>
							</div>
						</div>
					...
					<!-- Modal Footer Actions (Comfortable Buttons) -->
	...
	jQuery(document).ready(function($) {
		...
		function openAddModal() {
			...
			$('#fed_modal_subtitle').text('Configure title, icon, and role permissions.');
			...
			$('#fed_slug_hint').text('Unique identifier used for URL binding.');
		...
		$sortableList.on('click', '.fed-btn-edit', function(e) {
			...
			$('#fed_modal_subtitle').text('Update navigation attributes and permissions.');
		...
		$('#fed_modal_menu_form').on('submit', function(e) {
			...
			$.ajax({
				...
				error: function() {
					$('.preview-area').addClass('hide');
					$submitBtn.prop('disabled', false).removeClass('opacity-75');
					showToast('An unexpected network or server error occurred.', true);
				}
			});
		});

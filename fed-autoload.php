...
if ( ! function_exists( 'FED' ) ) {
	/**
	 * The below class will trigger the __construct to load the hooks.
	 *
	 * @return mixed
	 */
	function FED() {
		$adfuz = (object) array(
			'core'     => array(
				'hook'     => array(
					'action' => \FED\Hooks\Action\Action::get_instance(),
					'filter' => \AdFuz\Hooks\Filter\Filter::get_instance(),
				),
				'database' => array(
					'install' => \AdFuz\Database\Install::get_instance(),
				),
				'debug'    => array(
					'type' => 'test', // test, live
				),
			...
			'frontend' => array(
				'shortcode' => \AdFuz\Hooks\Action\Shortcode::get_instance(),
			),
			'request'  => new \AdFuz\Requests\Request(),
		);

		return apply_filters( 'fed/core', $adfuz );
	}
}
...
add_action(
	...
	function () {
		global $FED;
		do_action( 'fed/function/core:before' );
		$FED = FED();
		do_action( 'fed/function/core:after' );
	},
	10
);
